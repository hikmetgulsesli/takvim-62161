...
function loadDayNotes(): Record<string, string> {
  try {
    const saved = localStorage.getItem('takvim-day-notes');
...
export function CalendarProvider({ children }: { children: ReactNode }) {
  ...
  const updateNoteForDate = useCallback((date: Date, note: string) => {
    setState(prev => {
      const dateStr = formatDate(date);
      const newDayNotes = { ...prev.dayNotes, [dateStr]: note };
      localStorage.setItem('takvim-day-notes', JSON.stringify(newDayNotes));
      return { ...prev, dayNotes: newDayNotes };
    });
  }, []);
  ...
  const value = useMemo(() => ({
    ...
    getNoteForDate,
    updateNoteForDate,
  }), [state, navigateMonth, goToToday, selectDate, setView, addEvent, updateEvent, deleteEvent, getEventsForDate, getNoteForDate, updateNoteForDate]);
